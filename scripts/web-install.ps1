...
if ($localSetup -and (Test-Path -LiteralPath $localSetup)) {
    ...
    try {
        if ($Install) {
            & (Resolve-Path $localSetup).Path -Install -Mode:$Mode -SkipOptional:$SkipOptional -Quiet:$Quiet
        } else {
            & (Resolve-Path $localSetup).Path
        }
    } finally {
